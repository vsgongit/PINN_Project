...
def train(
    ...
    # physics parameters
    if learn_alpha:
        alpha_raw = nn.Parameter(torch.log(torch.tensor(alpha_prime_val + 1e-6)))
        alpha_raw = alpha_raw.to(device)
        alpha_param = alpha_raw
    else:
        alpha_param = torch.tensor(alpha_prime_val, device=device)

    if learn_beta:
        beta_raw = nn.Parameter(torch.log(torch.tensor(beta_prime_val + 1e-6))).to(device)
        beta_param = beta_raw
    else:
        beta_param = torch.tensor(beta_prime_val, device=device)

    # if learnable, put into optimizer
    ...
    for epoch in range(epochs):
        ...
        for noisy, clean, p0, tprime, dt in pbar:
            ...
            p0 = p0.to(device)  # [B,1,1]
            # expand tprime to batch
            tprime = tprime.to(device).expand(noisy.shape[0], -1, -1).clone().requires_grad_(True)  # [B,1,T]
            with torch.cuda.amp.autocast(enabled=(device.startswith("cuda"))):
                pred = model(noisy)  # [B,1,T]
                loss_data = mse_loss(pred, clean)
                # physics loss
                p_hat_prime = pred / (p0.unsqueeze(-1) + 1e-8)
                # alpha/beta positive parametrization
                if learn_alpha:
            ...
            running["stft"] += (loss_stft.item() if isinstance(loss_stft, torch.Tensor) else 0.0)
        ...
        if learn_alpha:
            writer.add_scalar("train/alpha_prime", float(F.softplus(alpha_param).detach().cpu().item()), epoch)
        if learn_beta:
            writer.add_scalar("train/beta_prime", float(F.softplus(beta_param).detach().cpu().item()), epoch)

        # validation
        val_si = validate_and_checkpoint(model, val_loader, device, p0=None, writer=writer, epoch=epoch,
                                         lambda_phys=lambda_phys, alpha_param=alpha_param, beta_param=beta_param,
                                         learn_alpha=learn_alpha, learn_beta=learn_beta, checkpoint_dir=checkpoint_dir)
        # save best
        if val_si > best_val_si:
            ...
            save_checkpoint({
                ...
                "alpha_param": (alpha_param.detach().cpu() if learn_alpha else None),
                "beta_param": (beta_param.detach().cpu() if learn_beta else None),
                "val_si": val_si
...
def validate_and_checkpoint(model, val_loader, device, p0, writer, epoch, lambda_phys, alpha_param, beta_param, learn_alpha, learn_beta, checkpoint_dir):
    model.eval()
    total_si = 0.0
    total_mse = 0.0
    count = 0
    ...
    with torch.no_grad():
        for noisy, clean, p0_batch, tprime, dt in val_loader:
            ...
            clean = clean.to(device)
            p0_batch = p0_batch.to(device)
            # forward
            pred = model(noisy)
            batch_si = si_sdr(pred, clean).item()
            total_si += batch_si
